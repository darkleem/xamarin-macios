...
#pragma warning restore 0414
		...
		{
			return XcrunFind (app.Platform, app.IsSimulatorBuild, tool, out path);
		}
		...
		static bool XcrunFind (ApplePlatform platform, bool is_simulator, string tool, out string path)
		{
			...
			if (platform != ApplePlatform.None) {
				...
				case ApplePlatform.WatchOS:
					...
					break;
				default:
					throw ErrorHelper.CreateError (71, "?");
				}
			}
			args.Add ("-f");
			args.Add (tool);
		...
		{
			string lipo = FindTool (tool);
			if (lipo == null)
				throw ErrorHelper.CreateError (99, Errors.MX0099, "TODO: pending error.");
			if (RunCommand (lipo, arguments) != 0)
				throw ErrorHelper.CreateError (99, Errors.MX0099, "TODO: pending error.");
		}
